...
module.exports = (info, callback) => {
  ...
  if (method == "POST" || method == "PUT") {
    ...
    // For Creating orders
    assignFormParamsIfExist(formParams, info, 'side'); // Buy or Sell
    ...
    assignFormParamsIfExist(formParams, info, 'orders'); // Bulk Orders
    ...
    // Leverage trades
    assignFormParamsIfExist(formParams, info, 'leverage');

    requestInput['body'] = JSON.stringify(formParams); // Refer to https://www.bitmex.com/app/apiKeysUsage
    requestInput['headers'] = headers({path: path, verb: method, key: apikey, secret: apisecret, data: formParams});
  }
